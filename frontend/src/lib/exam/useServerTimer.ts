 ...
 * re-anchor the countdown against the original full baseline.
 *
...
export function useServerTimer(
  ...
  useEffect(() => {
    ...
    const tick = () => {
      ...
      // even when the tick lands a few ms after the anchor was captured, and 0
      // is reached only at the true deadline.
