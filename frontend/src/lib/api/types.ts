...
export interface Question {
  ...
  skills: string[];
  tags: string[];
  image_object_key?: string; // MinIO object key for an attached diagram
  image_url?: string; // Pre-signed GET URL (populated by the backend when an image is attached)
  created_at: string;
  updated_at: string;
}
...
export interface QuestionCreate {
  ...
  skills: string[];
  tags: string[];
  image_object_key?: string; // Set after a direct-to-MinIO upload
}

export type QuestionUpdate = Partial<QuestionCreate>;

export interface PresignedUpload {
  upload_url: string; // Pre-signed PUT URL the browser uploads to directly
  object_key: string; // Server-generated key to store on the question
  expires_in: number; // URL validity window in seconds
}
...
// ===== W3-F3 QUIZ SESSION (W3-F1 backend contract) =====
// Mirrors test-management-service src/schemas/session_schema.py. The session
// API exposes one sanitized question at a time (answer key stripped); the
// client never computes timing — server_now/expires_at are authoritative.

export interface SanitizedQuestionOption {
  option_id: number; // 1-indexed, auto-generated by question-service
  text: string;
}

/** A question as exposed to the candidate — answer fields stripped server-side. */
export interface SanitizedQuestion {
  id: string; // MongoDB ObjectId
  type: QuestionType;
  question_text: string;
  options?: SanitizedQuestionOption[] | null;
  difficulty?: QuestionDifficulty | null;
  image_url?: string | null;
}

/** Response contract for POST /v1/api/sessions (consumed server-side by the page). */
export interface SessionOut {
  session_id: string;
  session_token: string;
  server_now: string; // ISO datetime
