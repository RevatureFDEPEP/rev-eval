...
export async function createTestSession(data: TestSessionCreate): Promise<TestSession> {
  ...
  const response = await api.post<TestSession & { id: string }>('/v1/api/test-sessions/', data);
...
export async function getTestSession(sessionId: string): Promise<TestSession> {
  const response = await api.get<TestSession & { id: string }>(`/v1/api/test-sessions/${sessionId}`);
...
export async function getTestSessionBySubmission(submissionId: number): Promise<TestSession> {
  const response = await api.get<TestSession & { id: string }>(`/v1/api/test-sessions/by-submission/${submissionId}`);
