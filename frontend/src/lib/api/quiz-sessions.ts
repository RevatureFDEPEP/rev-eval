...
// Backend returns `id` instead of `session_id` — map on the way out
type RawSessionResponse = Omit<TestSession, 'session_id'> & { id: string; session_id?: string };
...
export async function createTestSession(data: TestSessionCreate): Promise<TestSession> {
  ...
  const response = await api.post<RawSessionResponse>('/v1/api/test-sessions/', data);
...
export async function getTestSession(sessionId: string): Promise<TestSession> {
  const response = await api.get<RawSessionResponse>(`/v1/api/test-sessions/${sessionId}`);
...
export async function getTestSessionBySubmission(submissionId: number): Promise<TestSession> {
  const response = await api.get<RawSessionResponse>(`/v1/api/test-sessions/by-submission/${submissionId}`);
