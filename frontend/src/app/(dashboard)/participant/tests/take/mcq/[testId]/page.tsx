...
export default function QuizTestPage({ params }: QuizTestPageProps) {
  ...
  // Stable ref lets useTimer hold one stable callback while handleTimeExpired updates each render
  const onTimeExpiredRef = useRef<() => void | Promise<void>>(async () => {});
  const stableOnTimeExpired = useCallback(() => { onTimeExpiredRef.current(); }, []);

  const timer = useTimer({
    durationSeconds: totalDuration || 0,
    testId: isNaN(testId) ? '' : testId.toString(),
  ...
  const handleSubmitPartA = useCallback(async () => {
    ...
    try {
      ...
      await submitPartA({ session_id: sessionId, answers: answersArray });
  ...
  const handleFinalSubmit = useCallback(async () => {
    ...
    try {
      ...
      await submitPartB({ session_id: sessionId, answers: answersArray });
  ...
  // Keep ref in sync so stableOnTimeExpired always invokes the latest handleTimeExpired
  useEffect(() => {
