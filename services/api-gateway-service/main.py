...
ROUTES = [
    ...
    {"pattern": r"^/reports(/.*)?$", "service": "reporting-and-analytics-service"},
]
...
@app.api_route("/{path:path}", methods=["GET", "POST", "PUT", "DELETE", "PATCH", "OPTIONS"])  # noqa: E501
...
@app.api_route("/{service_name}/{path:path}", methods=["GET", "POST", "PUT", "DELETE", "PATCH", "OPTIONS"])  # noqa: E501
async def legacy_gateway(service_name: str, path: str, request: Request):
    ...
    try:
        ...
        raise HTTPException(status_code=503, detail=f"Cannot connect to service: {str(e)}")  # noqa: E501
    except Exception as e:
        print(f"❌ ERROR: {str(e)}")
        import traceback
        traceback.print_exc()
