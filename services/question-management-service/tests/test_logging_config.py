...
def test_formatter_emits_required_fields():
    formatter = JsonFormatter("question-management-service")
    output = json.loads(formatter.format(_make_record()))
    assert output["level"] == "INFO"
    assert output["service"] == "question-management-service"
...
def test_setup_logging_installs_single_stdout_handler():
    setup_logging("question-management-service", "INFO")
    root = logging.getLogger()
...
def test_setup_logging_respects_level():
    setup_logging("question-management-service", "DEBUG")
