"""Question-bank seed fixtures (single source of truth).

The ``QUESTIONS`` list below was extracted verbatim from
``seed_rag_context_questions.py`` so it can be reused without importing that
module (which pulls in ``httpx`` — absent from the service's production image).

Two consumers share this list:
  * ``seed_rag_context_questions.py`` — the standalone HTTP seeder.
  * ``src/db/seed.py`` — the guarded, idempotent startup seeder (W5-F4).

This module has no third-party imports so it is safe to import inside the
service at startup. Fixture *content* is intentionally not modified here; the
known ``true_false`` encoding inconsistency (some rows use ``[0]`` / ``[True]``
/ ``[False]``) is tracked separately (W5-F3) and tolerated by the startup
seeder.
"""
