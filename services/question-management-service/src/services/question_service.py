...
class QuestionService:
    ...
    def convert_options_to_stored_format(options: list[OptionCreate] | None) -> list[Option] | None:
        ...
        return [Option(option_id=idx, text=opt.text) for idx, opt in enumerate(options, start=1)]
        ...
        try:
            ...
            if question_data.get("options"):
                ...
                question_data["options"] = [
                    opt.model_dump()
                    for opt in QuestionService.convert_options_to_stored_format(options_create)
                ]
    ...
    async def update_question(qid: str, data: QuestionUpdate) -> bool:
        ...
        update_data = {
            k: v for k, v in data.model_dump().items() if v is not None and k != "updated_at"
        }
    ...
    def _validate_update_for_type(
        ...
        if question_type == QuestionType.MCQ.value:
            ...
            correct_answers = update_data.get("correct_answers") or existing_question.get(
                "correct_answers", []
            )

            if options and correct_answers:
                valid_option_ids = [
                    opt["option_id"] if isinstance(opt, dict) else opt.option_id for opt in options
                ]
