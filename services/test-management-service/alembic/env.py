...
def include_object(object, name, type_, reflected, compare_to):
    """Restrict autogenerate to tables this service owns.

    The Postgres database (eval_ai_dev) is shared with user-service, whose
    tables (e.g. `users`) are not in this metadata — without this filter,
    autogenerate would emit drop_table ops for them.
    """
    if type_ == "table":
