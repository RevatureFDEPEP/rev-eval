...
| W5-F3 | Render legacy option-less `true_false` questions | W3-F6 / debt §7 | ✅ Completed | [detail](features/w5-f3-legacy-true-false-rendering.md) | Dedicated `take/TrueFalseQuestion.tsx` (True→[1]/False→[0]; scores via exact-match bool↔int, no backend change). Finding: option-less is canonical, not legacy. |
| W5-F4 | Auto-seed the question bank in Docker Compose | W3-F1 | ✅ Completed | [detail](features/w5-f4-compose-question-bank-seed.md) | Branch `richardh-feat-W5F4`: guarded idempotent startup seed in QMS (`src/db/seed.py`), gated by `SEED_QUESTION_BANK` (default true, compose-set). Reuses fixtures via `src/db/seed_data.py`. Verified clean DB → 28 seeded (3 encoding-defect fixtures skipped) → `GET /questions/` non-empty; re-run no-op. |
